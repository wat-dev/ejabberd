...
wait_for_auth({xmlstreamelement, El}, StateData) ->
	...
	{auth, _ID, set, {U, P, D, R}} ->
		...
		true ->
			...
			{true, AuthModule} ->
				...
				ejabberd_hooks:run(c2s_auth_success, StateData#state.server,
					[IP, #jid{user = U, server = StateData#state.server, resource = ""}, "legacy"]),
			    SID = {now(), self()},
		...
		_ ->
			...
			JID == error ->
			    ?INFO_MSG(
			       "Forbidden legacy authentication for "
		...
		{xmlelement, "message", _, _} ->
			case (string:substr(To#jid.lserver, 1, 11) == "conference.") or
				...
				false ->
					...
					?SETS:is_element(SFrom, StateData#state.pres_f) orelse
					?SETS:is_element(BSFrom, StateData#state.pres_f);
				_ ->
					true
			end;
		_ ->
